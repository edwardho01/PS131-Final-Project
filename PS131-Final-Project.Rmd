...
Below, we will be removing some unneeded variables, renaming some columns, and other techniques for needed clarity in reading the data. Note, there is no direct need to remove missing/blank observations since I have already made sure they were not included through excel and before importing the csv, but for certainty, I'll rerun the process and check again below.
...
* Remove the start_time and state column since I had already filtered that completely in excel. In most cases, wind direction seems pretty negligable as well so it wouldn't be something worth much towards severity in my opinion.
...
I split my data using a proportion of 70/30, where the training data is 70% of my total data, and the testing data covers the other 30%. Because I had a large imbalance in the response variable, severity, then I must use stratified sampling (which you can see in the EDA portion).
...
The first thing we can notice already is the that crossing, traffic signals, and stop related traffic incidents are much more frequent than bumps and railway accidents. Therefore, we might be able to say that since there is a lower amount of railway and bump accidents in both categories, it might be two variables that won't probably be too important in determining the accidents. Additionally, it would make sense that these two factors are something that doesn't seem exclusive to severe accidents as well based off of experience. Being a daily driver myself, most accidents just generally tend to stray away from bumps and railways as they aren't populated parts of the road. 

### Distance Correlation with weather conditions?

```{r}
accidents_train %>%
  gather(temperature_f,wind_chill_f,humidity_percent,pressure_in,visibility_mi,wind_speed_mph,precipitation_in,-distance_mi, key = "var",value = "value") %>%
  ggplot(aes(x = value, y = distance_mi)) + geom_point() + facet_wrap(~ var, scales = "free")
```

As seen here, there doesn't seem to be any apparent correlation between the distance with any of the numerical weather conditions. Therefore, it doesn't seem as though weather conditions are too valuable in determining the severity of an accident IN TERMS OF STRICTLY distance, but it's important to note that distance isn't the only thing that accounts for severity, so we should keep that mind moving on. 
...
* Finally, as I noted before, a valuable way to deal with the class unbalance is to up sample. This means I will randomly sample with replacement our under-represented class until it matches about 50% (in my case) of the more abundance class. The reason for doing this is because if we don't up sample, then our model will continually predict severity 2 because that's the only data that the model is trained to predict since there's almost no appearances of severity 4 accidents, meaning that the models predictive power will be extremely low.
...
Again, I tuned both my boosted trees and random forest to similar hyperparameter ranges because I want to compare the two models performance in a relatively similar fashion. 
...
I want to emphasize that I'm mainly focusing on roc_auc because accuracy can be an extremely skewed value here especially since we have such a large class inbalance and naturally the models will produce a high accuracy due to the fact.
...
To make some final remarks, I believe that boosted trees and random forests will still function the best in terms of future analysis because k-nearest neighbors has obvious limitations, as such with logistic and singular decisions trees. Although my roc_auc wasn't impressive by any means, with the two best models producing only ~66% roc_auc, it still fit my testing data quite well meaning my model was by no means over fitted. But, as shown in logistic regression pertaining to the model, we must take the accuracy with a grain of salt because since we have so many severity 2 observations, all our models are clearly going to mainly predict severity 2 therefore leading to a high accuracy since getting all or some of the severity 4 prediction wrongs will not hurt accuracy by all too much. Overall, I am pretty happy with the outcome of the models, and would like to make some changes in the future in terms of data set range and even explore other states and years as well.
